...
void fromRPN(char *input, char *output)
{
    ...
    head = NULL;
    while (*input)
    { // пока есть символы строке
        c = (*input);
        if (c >= '0' && c <= '9' || c == '.')
        {                       // если текущий символ часть числа
        ...
        {               // если читаем знак операции
            ...
            { // если приоритет для 1-го элемента меньше
                temp[0] = '(';
                temp[1] = '\0'; // берем выражение в скобки
            ...
            { // если приоритет для 2-го элемента меньше
                strcat(temp, "(");
