...
class Store {
public:
    Store() : rootNode(nullptr) {}
    void writeNode(linkType& link, Node<recordType, linkType>& node)
    {
        link = std::make_shared<Node<recordType, linkType>>(node->N);
        *link = node;
    }
    Node<recordType, linkType> readNode(linkType& link)
    {
        Node<recordType, linkType> node(rootNode->N);
        node = *link;
        return node;
    }
private:
    std::shared_ptr<Node<recordType, linkType>> rootNode;
};
...
public:
    ...
    linkFS writeIndex;
    Store()
    : N{0}, rootIndex{0}, writeIndex{0}, sizeBlock{0}, fileName{0}
    {
    ...
    Node<recordType, linkFS> readNode(linkFS& index)
    {
        Node<recordType, linkFS> node(N);
